...
class WorkerAssignmentsView extends StatelessWidget {
  ...
  final WorkerAssignmentsController controller =
      Get.put(WorkerAssignmentsController());
  ...
  Widget _buildAssignmentCard(BuildContext context, dynamic assignment) {
    // 🐛 Debug: Print the assignment to see its actual structure
    print('📦 Assignment data: $assignment');
    print('📦 Assignment keys: ${assignment.keys}');
    
    // Try multiple possible field names for work order
    final workOrderCode = assignment['work_order_code']?.toString() ??
        assignment['workOrderCode']?.toString() ??
        assignment['work_order']?.toString() ??
        (assignment['work_order_id'] is Map 
            ? assignment['work_order_id']['code']?.toString() 
            : null) ??
        'Unknown Order';
    
    final workOrderTitle = assignment['work_order_title']?.toString() ??
        assignment['workOrderTitle']?.toString() ??
        (assignment['work_order_id'] is Map 
            ? assignment['work_order_id']['title']?.toString() 
            : null) ??
        '';
    
    final stageName = assignment['stage_name']?.toString() ??
        assignment['stageName']?.toString() ??
        (assignment['work_order_stage_id'] is Map 
            ? assignment['work_order_stage_id']['stage_name']?.toString() 
            : null) ??
        (assignment['work_order_stage_id'] is Map 
            ? assignment['work_order_stage_id']['name']?.toString() 
            : null) ??
        'Unknown Stage';

    // Try multiple possible field names for quantities
    final quantity = int.tryParse(
        assignment['assigned_quantity']?.toString() ??
        assignment['assignedQuantity']?.toString() ??
        assignment['quantity']?.toString() ?? '0') ?? 0;
    
    final outputQty = int.tryParse(
        assignment['worker_output_quantity']?.toString() ??
        assignment['workerOutputQuantity']?.toString() ??
        assignment['output_quantity']?.toString() ?? '0') ?? 0;
    
    final approvedQty = int.tryParse(
        assignment['admin_approved_quantity']?.toString() ??
        assignment['adminApprovedQuantity']?.toString() ??
        assignment['approved_quantity']?.toString() ?? '0') ?? 0;
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            // Output + Approved Qty if submitted/approved
            if (isSubmitted) ...[
              const SizedBox(height: 8),
              Row(
                children: [
                  Expanded(
                    child: _buildInfoItem('Output Qty', '$outputQty',
                        color: Colors.blue),
                  ),
                  if (status == 'approved')
                    Expanded(
                      child: _buildInfoItem('Approved Qty', '$approvedQty',
                          color: Colors.green),
                    ),
                ],
              ),
  ...
  void _showSubmitDialog(BuildContext context, dynamic assignment) {
    showDialog(
      context: context,
      builder: (context) => _SubmitWorkDialog(
        assignment: assignment,
        onSuccess: controller.fetchAssignments,
      ),
    );
  }
}
...
class _SubmitWorkDialogState extends State<_SubmitWorkDialog> {
  ...
  Future<void> _loadInventory() async {
    try {
      // ✅ Your payload has direct IDs:
      // "work_order_id": "101",
      // "work_order_stage_id": "456",
      final workOrderId = widget.assignment['work_order_id']?.toString();
      final stageId = widget.assignment['work_order_stage_id']?.toString();
      final assignmentId = widget.assignment['id']?.toString();

      if (workOrderId != null && stageId != null && assignmentId != null) {
        final response =
            await ApiService.getStageInventoryAllocations(workOrderId, stageId);
          ...
          setState(() {
            ...
            for (var alloc in inventoryAllocations) {
              ...
              if (allocId.isNotEmpty) {
                returnedControllers[allocId] =
                    TextEditingController(text: '0');
                brokenControllers[allocId] =
                    TextEditingController(text: '0');
              }
            }
    ...
    try {
      ...
      for (var alloc in inventoryAllocations) {
        ...
        if (returnedController != null && brokenController != null) {
          final quantityReturned =
              int.tryParse(returnedController.text) ?? 0;
          final quantityBrokenLost =
              int.tryParse(brokenController.text) ?? 0;
          ...
          children: [
            ...
            if (inventoryAllocations.isNotEmpty) ...[
              ...
              ...inventoryAllocations.map((alloc) {
                ...
                final quantityProvided =
                    alloc['quantity_provided'] ?? 0;
